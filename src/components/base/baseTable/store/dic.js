...
import { isArray, isPlainObject, isFunction, merge } from 'lodash'
...
export function dicToCommon(dic, props = {}) {
  ...
  dic.forEach((dicItem) => {
    dicItem.label = isFunction(label) ? label(dicItem) : dicItem[label]
    dicItem.value = isFunction(value) ? value(dicItem) : dicItem[value]
    if (dicItem[children]) {
...
export function createDic(...args) {
  ...
  let repeatKeys = []
  dics.reduce((prev, next) => {
    const keys = Object.keys(next.state)
    repeatKeys = repeatKeys.concat(keys.filter(key => prev.includes(key)))
    return prev.concat(keys)
  }, [])
  repeatKeys.length && console.error(`以下dic重复设置：${repeatKeys.join('、')}`)
